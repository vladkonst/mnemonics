-- +goose Up
-- Extend role CHECK constraint to include 'manager'.
CREATE TABLE users_new (
    telegram_id             BIGINT PRIMARY KEY,
    role                    TEXT NOT NULL DEFAULT 'unknown'
                                CHECK(role IN ('unknown','student','teacher','manager')),
...
INSERT INTO users_old SELECT telegram_id,
    CASE WHEN role = 'manager' THEN 'unknown' ELSE role END,
    subscription_status, university_code, pending_payment_id, username, language, timezone,
    notifications_enabled, last_activity_at, created_at
FROM users;
DROP TABLE users;
